...
class PegLink(extl.ExtlinkInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='box', name='peg_link', enable_cc=True):
        ...
        self.jlc = jl.JLChain(pos=cpl_end_pos, rotmat=cpl_end_rotmat, homeconf=np.zeros(1), name='peg_link_jlc')
        self.jlc.jnts[1]['loc_pos'] = np.array([.0, .0, .0])
        self.jlc.jnts[1]['loc_motionax'] = np.array([0, 0, 1])
        self.jlc.lnks[0]['name'] = "peg_link"
        self.jlc.lnks[0]['loc_pos'] = np.zeros(3)
        self.jlc.lnks[0]['mesh_file'] = os.path.join(this_dir, "meshes", "ball_peg.stl")
...
if __name__ == '__main__':
    ...
    grpr = PegLink(enable_cc=True)
    grpr.gen_meshmodel().attach_to(base)
    grpr.gen_stickmodel(toggle_jntscs=True).attach_to(base)
    grpr.fix_to(pos=np.array([0, .3, .2]), rotmat=rm.rotmat_from_axangle([1, 0, 0], .5))
