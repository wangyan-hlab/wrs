...
class FR5_robot(ri.RobotInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='fr5', homeconf=np.zeros(7), enable_cc=True):
        ...
        self.arm.jnts[6]['loc_rotmat'] = rm.rotmat_from_euler(-math.pi / 2.0, 0, 0)
        self.arm.jnts[6]['loc_motionax'] = np.array([0, 0, 1])
        self.arm.jnts[6]['motion_rng'] = np.array([-175, 175]) * math.pi / 180
        self.arm.jnts[7]['loc_pos'] = np.array([0, 0, .102+.04])
        self.arm.jnts[7]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, 0)
        self.arm.jnts[7]['loc_motionax'] = np.array([0, 1, 0])
